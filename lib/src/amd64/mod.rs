...
pub enum Operand {
    ...
    Address(SegmentOverride,Register,Register,usize,(u64,usize)), // Segment Override, Base, Index, Scale, Disp
    Optional,
}
...
fn read_effective_address(mode: Mode, seg: SegmentOverride, tail: &mut Tail,
    ...
    match (mod_,rm & 0b111) {
        ...
        (0b00,0b100) =>
            tail.sib(mod_,seg,rex,addrsz),
        (0b00,0b101) if mode == Mode::Long =>
            Ok(Operand::Address(seg,Register::None,Register::None,0,(ip + sign_ext_u32(try!(tail.read_u32()),addrsz),32))),
        (0b00,0b101) if mode != Mode::Long =>
